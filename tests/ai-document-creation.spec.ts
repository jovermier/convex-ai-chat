...
test.describe("AI Document Creation", () => {
  ...
  test("should create a document when asking AI to write one", async ({ page }) => {
    ...
    await expect(page.getByRole("textbox", { name: "Document title..." })).toBeVisible({ timeout: 5000 });

    // Set document title
    const titleInput = page.getByRole("textbox", { name: "Document title..." });
    await titleInput.fill("AI Test Document");
    ...
    // The response should appear in a message bubble
    const aiResponse = page
      .locator('div[class*="bg-muted"]')
      .filter({ hasText: /created|wrote|here'?s|document|plan/i });
    ...
    const hasHeading = await page.locator(".ProseMirror h1, .ProseMirror h2, .ProseMirror h3").count();
  ...
  test("should show user message immediately, then AI response", async ({ page }) => {
    ...
    await page.getByRole("button", { name: /new document/i }).click();
    await expect(page.getByRole("textbox", { name: "Document title..." })).toBeVisible({ timeout: 5000 });
    await page.getByRole("textbox", { name: "Document title..." }).fill("Timing Test");
    ...
    const aiResponse = page.locator('div[class*="bg-muted"]').filter({ hasText: /testing|introduction|software/i });
    await expect(aiResponse).toBeVisible({ timeout: 60000 });
  ...
  test("should append content when asked", async ({ page }) => {
    ...
    await page.getByRole("button", { name: /new document/i }).click();
    await page.getByRole("textbox", { name: "Document title..." }).fill("Append Test");
    ...
    // Verify AI responds
    const aiResponse = page.locator('div[class*="bg-muted"]').filter({ hasText: /appended|added|best practices/i });
    await expect(aiResponse).toBeVisible({ timeout: 60000 });
  ...
  test("should handle multiple edits in one response", async ({ page }) => {
    ...
    await page.getByRole("button", { name: /new document/i }).click();
    await page.getByRole("textbox", { name: "Document title..." }).fill("Multi-Edit Test");
    ...
    const aiResponse = page.locator('div[class*="bg-muted"]').filter({ hasText: /updated|modified|sections/i });
    await expect(aiResponse).toBeVisible({ timeout: 60000 });
...
test.describe("AI Error Handling", () => {
  ...
  test("should show error message when AI fails", async ({ page }) => {
    // Create document
    await page.getByRole("button", { name: /new document/i }).click();
    await page.getByRole("textbox", { name: "Document title..." }).fill("Error Test");
